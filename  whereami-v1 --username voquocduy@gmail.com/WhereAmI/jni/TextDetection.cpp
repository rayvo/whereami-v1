...
/*
    ...
    along with DetectText.  If not, see <http://www.gnu.org/licenses/>.lk
*/
...
vector<pair<CvPoint,CvPoint> > findBoundingBoxes(vector<vector<Point2d> > & components,
                                                           vector<Chain> & chains,
                                                           vector<pair<Point2d,Point2d> > & compBB,
                                                           IplImage * output) {
    vector<pair<CvPoint,CvPoint> > bb;
...
void renderChains (IplImage * SWTImage,
    ...
    cout << componentsRed.size() << " components after chaining";
    cout << std::endl;
...
void SWTMedianFilter (IplImage * SWTImage,
    ...
    for (std::vector<Ray>::iterator rit = rays.begin(); rit != rays.end(); rit++) {
        ...
        float median = (Point2d (rit->points[rit->points.size()/2])).SWT;
        //RAYVO float median = (rit->points[rit->points.size()/2]).SWT;
        for (std::vector<Point2d>::iterator pit = rit->points.begin(); pit != rit->points.end(); pit++) {
